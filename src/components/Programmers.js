...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const match = programmersList.filter(programmer => {
      return programmer.id === featuredProgrammerId;
    }); 
    return match[0].name;
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { /* in here set the featured id to be dev.id */ setFeaturedProgrammerId(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
